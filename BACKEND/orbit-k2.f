...
C---- Leer parámetros orbitales desde línea de comando
...
C---- Validación de altura mínima de la órbita
      RP = E(1) * (1 - E(2))
      IF (RP .LT. 1.0234375D0) THEN
         WRITE(*,*) 'ALTITUDE OF SATELLITE IS BELOW 150 KM'
      END IF

C---- Calcular el período orbital
...
C---- Integración temporal hasta completar el número de revoluciones
      DO WHILE (T .LT. PER)
         CALL RK78(T, X, N, H, HMIN, HMAX, TOLRK, R, B, F, STJJIN)
         TN = TN + 1
         CALL VAPELE(E, X)
         CALL CCIUEO(E, 2)
         CALL PRINT_LINE(TN, E, X, T)
...
C Posar els parametres iguals a totes les rutines que els tinguin !!
        PARAMETER (XMU=1.D0,RE=1.D0,XJ2=0.00108263D0)
        DIMENSION X(*),F(*)
        F(1)=X(4)
        F(2)=X(5)
        F(3)=X(6)
        ...
        P2=1.D0/(P1**3.D0)
        ZR=(3.D0*(X(3)*X(3)))/(P1**2)
        CTA=(1.5D0*XMU*RE*RE*XJ2)/(P1**4.D0)
        F(4)=-X(1)*XMU*P2-CTA*(1.D0-ZR)*(X(1)/P1)
        F(5)=-X(2)*XMU*P2-CTA*(1.D0-ZR)*(X(2)/P1)
        F(6)=-X(3)*XMU*P2-CTA*(3.D0-ZR)*(X(3)/P1)
        RETURN
        END
